...
public class FeedbackService 
{
    ...
    public List<Feedback> getAllFeedback() {
        try (Connection conn = source.getConnection()) {
            try (PreparedStatement stat = conn.prepareStatement("SELECT * FROM feedback INNER JOIN gebruikers ON feedback.GebruikerID = gebruikers.GebruikerID")) {
    ...
    public Feedback getFeedback(@PathParam("FeedbackID") int id) {
        try (Connection conn = source.getConnection()) {
            try (PreparedStatement stat = conn.prepareStatement("SELECT * FROM feedback INNER JOIN gebruikers ON feedback.GebruikerID = gebruikers.GebruikerID WHERE feedback.FeedbackID = ?")) {
                stat.setInt(1, id);
                try (ResultSet rs = stat.executeQuery()) {
                    if (rs.next()) {
                        Feedback m = new Feedback();
