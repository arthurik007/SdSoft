...
public class MeldingService 
{
    ...
    public Response addMelding(Melding m) {
        try (Connection conn = source.getConnection()) {
            ...
            try (PreparedStatement stat = conn.prepareStatement("INSERT INTO meldingen VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
    ...
    public void updateMelding(@PathParam("MeldingID") int id, Melding m) {
        try (Connection conn = source.getConnection()) {
            ...
            try (PreparedStatement stat = conn.prepareStatement("UPDATE meldingen SET Soort = ?, Straat = ?, Stad = ?, Straatnummer = ?, Omschrijving = ?, Latitude = ? , Longitude = ? , GebruikerID = ? WHERE MeldingID = ?")) {
...
public class MeldingService 
{
    ...
    @Produces(MediaType.APPLICATION_JSON)
     ...
     public List<Melding> getAllMeldingen() {
        try (Connection conn = source.getConnection()) {
            try (PreparedStatement stat = conn.prepareStatement("SELECT * FROM meldingen")) {
                try (ResultSet rs = stat.executeQuery()) {
                    List<Melding> results = new ArrayList<>();
                    while (rs.next()) {
                        Melding m = new Melding();
    ...
    public void updateMelding(@PathParam("MeldingID") int id, Melding m) {
        try (Connection conn = source.getConnection()) {
            ...
            try (PreparedStatement stat = conn.prepareStatement("UPDATE meldingen SET Soort = ?, Straat = ?, Stad = ?, Straatnummer = ?, Omschrijving = ?, Latitude = ? , Longitude = ? , Gebruikernaam = ? WHERE MeldingID = ?")) {
                stat.setString(1, m.getSoort());
                stat.setString(2, m.getStraat());
                stat.setString(3, m.getStad());
                stat.setInt(4, m.getStraatnummer());
