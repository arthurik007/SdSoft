    ...
    if (navigator.geolocation) {
        ...
        function hasPosition(position) {
            ...
            google.maps.event.addListener(map, 'click', function(event) {
                deleteOverlays();
                document.getElementById("latFld1").value = event.latLng.lat();
                document.getElementById("lngFld1").value = event.latLng.lng();
                ...
                geo.geocode({'latLng': latlng}, function(results, status) {
                    if (status == google.maps.GeocoderStatus.OK) {
                        ...
                        document.getElementById("st1").value = getfirstPart(adres);
                        document.getElementById("stn1").value = getSecondPart(adres);
                        document.getElementById("std1").value = getThirdPart(deelTwee) + ' ' + getFourthPart(deelTwee);
